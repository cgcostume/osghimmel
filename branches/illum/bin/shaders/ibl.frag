...
float minEnvAffection = 0.0;
float maxEnvAffection = 1.0;
...
void main() {
	...
	//vec2 noiseIndex = vec2(nnNoise, nnNoise + pos.zyx);
	...
	for (int i = 0; i < 16; ++i) {
		int index = int(floor(((normal.x + 1.0) / 2.0) * 1000.0));
		//vec3 noise = texture2D(noiseMap, pos.xy * float(i)).rgb;
		//vec3 noise = kernel[index];
		
		vec3 noise = kernel[i];
		
		//noise.rg = noise.rg * 2.0 - 1.0;
		//noise = normalize(noise);
		//noise.g = (noise.g + 1.0) / 2.0;
		v = tbn * noise;
		v = normalize(v);
		...
		//color = mix(color, envColor, dirWeight / float(i+1));
		color = mix(color, envColor, pow(dot(normal, v), 2.0));
	}
